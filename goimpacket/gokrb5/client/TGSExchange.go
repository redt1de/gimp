...
import (
	...
	"github.com/redt1de/gimp/goimpacket/gokrb5/iana/flags"
	"github.com/redt1de/gimp/goimpacket/gokrb5/iana/nametype"
	"github.com/redt1de/gimp/goimpacket/gokrb5/krberror"
...
// SPN format: <SERVICE>/<FQDN> Eg. HTTP/www.example.com
// The ticket will be added to the client's ticket cache
func (cl *Client) GetServiceTicketAsCCache(spn string) (*credentials.CCache, error) {
	// var tkt messages.Ticket
	var skey types.EncryptionKey
	if _, _, ok := cl.GetCachedTicket(spn); ok {
		// Already a valid ticket in the cache
		return nil, nil
	}
	princ := types.NewPrincipalName(nametype.KRB_NT_PRINCIPAL, spn)
...
// GetServiceTicketForUser uses constrained delegation to issue a ticket for a specific user and service.
func (cl *Client) GetServiceTicketForUserAsCCache(spn string, userRealm string, user types.PrincipalName) (*credentials.CCache, error) {
	// var tkt messages.Ticket
	// var skey types.EncryptionKey
	...
	tgt, sessionKey, err := cl.sessionTGT(cl.Credentials.Realm())
	if err != nil {
		return nil, fmt.Errorf("while getting TGT: %w", err)
	}
	userTGSReq, err := messages.NewS4U2SelfTGSReq(cl.Credentials.CName(), cl.Credentials.Realm(), cl.Config, tgt, sessionKey, false, user, userRealm)
	if err != nil {
		return nil, fmt.Errorf("assembling S4U2self request failed: %w", err)
	}
	_, userTGSRep, err := cl.TGSExchange(userTGSReq, cl.Credentials.Realm(), tgt, sessionKey, 0)
	if err != nil {
	...
	svcTGSReq, err := messages.NewS4U2ProxyTGSReq(cl.Credentials.CName(), realm, cl.Config, tgt, sessionKey, princ, false, userTGSRep.Ticket, user)
	if err != nil {
		return nil, fmt.Errorf("assembling S4U2proxy request failed: %w", err)
	}

	_, svcTGSRep, err := cl.TGSExchange(svcTGSReq, realm, tgt, sessionKey, 0)
	if err != nil {
		return nil, fmt.Errorf("while getting S4U2proxy ticket for user: %w", err)
	}

	return svcTGSRep.ToCCache()
}
